...
|------------|--------|------------|
| 2026-03-30 | **Cheaper-side experiment**: New `SIDE_SELECTION=cheaper` config. Instead of ML model picking UP/DOWN, always buys whichever side has the lower ask price. Regime flip disabled in this mode. All trades tagged `cheap_side` for clean analysis. Motivated by 17 days of shadow window data showing EE is the sole profit engine and model adds no directional value. $50 experiment to validate real EE rate and settlement WR without model. | — |
