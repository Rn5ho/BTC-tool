...
33. **Settlement uses Binance price, not Chainlink** (known — open): Real-time settlement compares `btc_start` vs `btc_end` using Binance candle prices, but Polymarket resolves using Chainlink BTC/USD. Binance and Chainlink can diverge by $20-50, causing our DB to record WIN when Polymarket actually resolved as LOSS (or vice versa). Gamma backfill catches these after the fact, but the live settlement P&L and Telegram alerts show the wrong outcome ~7.8% of the time. Chainlink RTDS stream exists but is only used for `btc_start` fallback, not for `btc_end` settlement. Fix: use Chainlink price from RTDS stream as the settlement price source instead of Binance.

34. **DB P&L was overstated by ~$139** (fixed 2026-03-08): P&L calculations used formula-estimated token counts and sell proceeds instead of actual CLOB fill data. Fixed: (1) token count at buy now uses `takingAmount` from CLOB response, (2) EE sell P&L now uses actual USDC received from sell response, (3) stale settlements try `response_json` for actual tokens. Historical trades before this fix still use formula estimates.
